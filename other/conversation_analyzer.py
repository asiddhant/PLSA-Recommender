...
def sentimentanalyzer(sentence,keysent):
    ...
    if pos_score>0.7:
        sentiment_score=5
    elif pos_score>0.5 and neg_score<0.5:
        sentiment_score=4
...
def moviesextractor(postagged,userprofiles,userid,movies):
    ...
    if max(movienames):
        ...
        if len(maxmatch)>1 and len(maxmatch)<15:
            ...
            #f.write("\n")
            return list(submovies.ix[eligible]['movieid']),list(submovies.ix[eligible]['title'])
        elif len(maxmatch)==1:
            #f.write(str(list(movies.ix[maxmatch]['title'])))
            #f.write("\n")
            return list(movies.ix[maxmatch]['movieid']),list(movies.ix[maxmatch]['title'])
        else:
            return [],[]
    else:
        return [],[]
...
def comparisonanalyzer(keysent,sentence,userprofiles,userid,tagged,sid,movies,starcast,common,genres):
    poscompwords=["more","better"]
    negcompwords=["less","worse"]
    if tagged[len(tagged)-1][1]=="JJR":
        if tagged[len(tagged)-1][1] in poscompwords:
            keysent,userprofiles=normalanalyzer(keysent,sentence,userprofiles,userid,tagged,"P",sid,movies,starcast,common,genres)
            return keysent,userprofiles
        elif tagged[len(tagged)-1][1] in negcompwords:
            keysent,userprofiles=normalanalyzer(keysent,sentence,userprofiles,userid,tagged,"N",sid,movies,starcast,common,genres)
            return keysent,userprofiles
    ...
    if detpresence(parts[1]):
        if comp[0] in poscompwords:
            keysent,userprofiles=normalanalyzer(keysent,p2,userprofiles,userid,parts[1],"N",sid,movies,starcast,common,genres)
            keysent,userprofiles=normalanalyzer(keysent,p1,userprofiles,userid,parts[0],"P",sid,movies,starcast,common,genres)
        if comp[0] in negcompwords:
            keysent,userprofiles=normalanalyzer(keysent,p2,userprofiles,userid,parts[1],"P",sid,movies,starcast,common,genres)
            keysent,userprofiles=normalanalyzer(keysent,p1,userprofiles,userid,parts[0],"N",sid,movies,starcast,common,genres)
    else:
        if comp[0] in poscompwords:
            keysent,userprofiles=normalanalyzer(keysent,p1,userprofiles,userid,parts[0],"P",sid,movies,starcast,common,genres)
            keysent,userprofiles=normalanalyzer(keysent,p2,userprofiles,userid,parts[1],"N",sid,movies,starcast,common,genres)
...
#        relwords[i]=porter_stemmer.stem_word(relwords[i])
    ...
    if relwords:
        exmovies,exmoviesn = moviesextractor(relwords,userprofiles,userid,movies)
        if exmovies:
...
def userinteraction(userid, sentences, usernames, umapping, adjlst, movies, actors, common):
    print("Entering user interation")
    numusers=3 ### THIS HAS BEEN HARDCODED. PLEASE PASS IT AS A PARAMETER IF NUMUSERS != 3
    stop_words=get_stop_words('en')
    wordnet_lemmatizer = WordNetLemmatizer()
    porter_stemmer = PorterStemmer()
    recentconv=[]
    ...
    for i in range(len(sentences)):
        ...
        try:    
            ...
            if len(userref)!=0:
                ...
                for tx in range(len(userref)):
                    ...
                    if index==0:
                        ...
                        if adjlst[int(userid[i])][int(userref[tx])] >= - numusers :
            ...
            print("I made a booboo because of the following: ")
            print(e)
...
def userinfluence(movieusermatrix, adjlst, numusers, maxmovid, threshold, mode, weights):
    movieusermatrix2=copy.deepcopy(movieusermatrix)
    print(adjlst)
    for i in range(0,numusers):
        for j in range(0,numusers):
            for k in range(0,maxmovid):
                movieusermatrix2[i][k]=movieusermatrix[i][k]+(adjlst[i][j]/(2.0*numusers))*(movieusermatrix[j][k]-movieusermatrix[i][k])
                #updateamt=(adjlst[i][j]/(2*numusers))*(movieusermatrix[j][k]-movieusermatrix[i][k])
    
        
        #init1=copy.deepcopy(movieusermatrix[i][:])
        #fin1=copy.deepcopy(movieusermatrix2[i][:])
    avgs=[0]*maxmovid
    ignore=[]
    for i in range(0,numusers):
        for j in range(0,maxmovid):
            if j in ignore:
                continue
            if movieusermatrix2[i][j]<threshold:
                ignore.append(j)
                avgs[j]=-30
                continue
            avgs[j]= avgs[j] + movieusermatrix2[i][j]*(weights[i]/float(sum(weights)))
    #avgs=np.divide(avgs,maxmovid)
    ...
    for i in range(0,numusers):
        tempPrint[i] = []
        for j in range(0,maxmovid):
            tempPrint[i] += [((j,movieusermatrix[i][j]),(j,movieusermatrix2[i][j]))]

        tempPrint[i] = sorted(tempPrint[i],key= lambda t:t[0][1],reverse=True)
    tempmovieData = plsa.getMovieData('C:/Users/nisyadav/AppData/Local/Programs/Python/Python35-32/Lib/collabrec/','data/movienames.csv')
    

    for u in tempPrint.keys():
        ctr  =0
        for (t1,t2) in tempPrint[u]:
            ctr += 1
            if ctr < 500:
                if avgs[t1[0]] > threshold:
                    print(ctr,t1[0]+1,"{0:.3f}".format(t1[1]),"{0:.3f}".format(t2[1]),"{0:.3f}".format(avgs[t1[0]]),tempmovieData[t1[0]+1]['name'],tempmovieData[t1[0]+1]['genre'])
            else:
                break;

        print("\n\n\n");
    
    print("Finding max movies now");
    maxm=max(avgs)
    if maxm==-30:
        raise ValueError("Rubbish1")
    #print(maxm)
    print("Found 1:",maxm);
    cnsns=maxm/3
...
def main_callable(conversation,irec,adjlst,userids):
    ...
    genre=[]
    print("Breakpoint_1",len(conversation))
